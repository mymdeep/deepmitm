...
public class HarCaptureFilter extends HttpsAwareFiltersAdapter {
    ...
    protected void captureRequestContent(HttpRequest httpRequest, byte[] fullMessage) {
        ...
        deep.com.deepproxy.log.Logger.single(0,"httpRequest="+httpRequest.getUri()+"   method="+httpRequest.getMethod());
        deep.com.deepproxy.log.Logger.single(0,"charset="+charset);
        deep.com.deepproxy.log.Logger.single(0,"full0="+fullMessage.length);
        deep.com.deepproxy.log.Logger.single(0,"contentType="+contentType);
        harEntry.getRequest().getPostData().setData(fullMessage);
        if (charset == null) {
