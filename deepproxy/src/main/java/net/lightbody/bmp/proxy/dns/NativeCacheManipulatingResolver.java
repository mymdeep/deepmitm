    ...
    public void setPositiveDNSCacheTimeout(int timeout, TimeUnit timeUnit) {
        try {
            ...
            if (timeout < 0) {
                ...
                java.security.Security.setProperty("networkaddress.cache.ttl", Long
                    .toString(TimeUnit.SECONDS.convert(timeout, timeUnit)));
            }
    ...
    public void setNegativeDNSCacheTimeout(int timeout, TimeUnit timeUnit) {
        try {
            ...
            if (timeout < 0) {
                ...
                java.security.Security.setProperty("networkaddress.cache.negative.ttl", Long
                    .toString(TimeUnit.SECONDS.convert(timeout, timeUnit)));
            }
        } catch (ClassNotFoundException | NoSuchFieldException | IllegalAccessException e) {
            log.warn("Unable to modify native JVM DNS cache timeouts", e);
